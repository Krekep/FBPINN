...
def sutherland_hodgman_clip(subjectPolygon: List[Tuple[float, float]], clipRect: Tuple[float, float, float, float]):
    """
...
def point_in_poly(xs: np.ndarray, ys: np.ndarray, poly_arr: np.ndarray) -> np.ndarray:
    """
...
def sample_points_in_polygon_with_holes(poly: List[Tuple[float, float]], holes: List[List[Tuple[float, float]]],
                                        n: int) -> np.ndarray[tuple[int, int], np.dtype[np.float32]]:
    """
    ...
    fill_ratio = max((poly_area - holes_area) / bbox_area, 0.01) if bbox_area > 0 else 0.01
    ...
    for _ in range(max_iters):
        ...
        mask = point_in_poly(rx, ry, poly_arr)
        for h_arr in holes_arr:
            mask &= ~point_in_poly(rx, ry, h_arr)
...
def dist(a: Sequence[float], b: Sequence[float]) -> float:
    ...
    """
    res = 0
    assert len(a) == len(b), "Can not calculate distance between points with different dimensions"
    for i, j in zip(a, b):
        res += (i - j) * (i - j)
    return math.sqrt(res)


def point_on_segment(a: Sequence[float], b: Sequence[float], p: Sequence[float]) -> bool:
    """
...
def point_on_polygon_edge(xs: np.ndarray, ys: np.ndarray, poly_arr: np.ndarray, tol=1e-9) -> np.ndarray:
    """
...
def sample_points_on_boundary(
    ...
    orig_arr = (
        ...
        if original_polygon is not None else None
    )
    ...
    for _ in range(max_iters):
        ...
        if block_rect is not None:
            ...
            on_bb = (
                    (np.abs(xs - xmin) < 1e-9) | (np.abs(xs - xmax) < 1e-9) |
                    (np.abs(ys - ymin) < 1e-9) | (np.abs(ys - ymax) < 1e-9)
            )
        ...
        if holes_arr:
            ...
            for h_arr in holes_arr:
                in_hole = point_in_poly(xs, ys, h_arr)
...
def segment_intersects_rect(a: tuple[float, float], b: tuple[float, float], block: list[float]) -> bool:
    """
    ...
    if (
            (a[0] < xmin and b[0] < xmin) or (a[0] > xmax and b[0] > xmax) or
            (a[1] < ymin and b[1] < ymin) or (a[1] > ymax and b[1] > ymax)
    ):
        return False
    ...
    if (xmin <= a[0] <= xmax and ymin <= a[1] <= ymax) or (xmin <= b[0] <= xmax and ymin <= b[1] <= ymax):
        return True
    return False


def polygon_intersects_rectangle(polygon: Optional[list[tuple[float, float]]], block: list[float]) -> bool:
    """
    Checking the intersection of a segment and a rectangle
