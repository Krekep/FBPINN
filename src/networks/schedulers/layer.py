...
class SequenceLayerScheduler(BaseLayerScheduler):
    ...
    def __init__(
        ...
        self.current_indices = list(range(self.current_left_bound, self.current_right_bound))
        self.is_changes = True
        self.frozen_indices = []
        ...
        if self.is_changes:
            ...
            if self.current_step % self.schedule == 0:
                if self.current_right_bound == self.n:
                    ...
                    self.current_ax = min(len(self.blocks_per_axis) - 1, self.current_ax + 1)
                    self.current_right_bound = min(
                        self.n, self.current_right_bound + self.blocks_per_axis[self.current_ax]
                    )
                    curr_lb = self.current_left_bound
                    new_lb = self.current_left_bound + self.blocks_per_axis[self.current_ax - 1]
                    self.current_left_bound = new_lb
                    self.frozen_indices = [i for i in range(curr_lb, new_lb)]
                range_changed = True
            if range_changed:
...
class GrowingColumnScheduler(BaseLayerScheduler):
    ...
    def step(self):
        ...
        self.current_col = min(
            self.current_col + 1, len(self.blocks_per_axis) - 1
        )
        self.current_right_bound = min(
            self.n,
            self.current_right_bound + self.blocks_per_axis[self.current_col]
        )
