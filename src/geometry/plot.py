...
from src.geometry.polygon_decomposition import PolygonBlock
...
def plot_decomposition2d(
    ...
    if poly_np.ndim == 2 and poly_np.shape[0] > 0:
        ax.plot(poly_np[:,0], poly_np[:,1], '-k', linewidth=1.2, label="outer contour")
    ...
    for block in decomposition:
        ...
        rect = Rectangle((x0,y0), width, height, linewidth=0.6, edgecolor=rect_edge_color, facecolor=face)
        ax.add_patch(rect)

        clipped_np = np.array(clipped)
        if clipped_np.shape[0] >= 3:
            ax.add_patch(MplPolygon(clipped_np, closed=True, fill=False, edgecolor='black', linewidth=0.8))

        if clipped_holes:
            for ch in clipped_holes:
                ch_np = np.array(ch)
                if ch_np.shape[0] >= 3:
                    ax.add_patch(MplPolygon(ch_np, closed=True, fill=False, edgecolor='red', linewidth=0.8, linestyle='--'))

        if data is not None:
            darr = np.array(data, dtype=float)
            if darr.ndim == 2 and darr.shape[1] >= 2 and darr.shape[0] > 0:
                ax.scatter(darr[:,0], darr[:,1], s=4, c='tab:blue', alpha=0.6, marker='.', label='_nolegend_')
    ...
    if xs and ys:
        ...
        dx = (xmax - xmin) * 0.14 + 1e-6; dy = (ymax - ymin) * 0.14 + 1e-6
    ...
    legend_elems = [
        ...
        Rectangle((0,0),1,1, facecolor=interior_face, edgecolor=rect_edge_color, label='interior block'),
        Rectangle((0,0),1,1, facecolor=boundary_face, edgecolor=rect_edge_color, label='boundary block'),
        Line2D([0],[0], marker='.', color='w', label='collocation pts', markerfacecolor='tab:blue', markersize=6)
    ]
