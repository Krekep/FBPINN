...
from src.geometry.geometry import polygon_intersects_rectangle
from src.networks import losses, metrics, optimizers, get_activation
from src.networks.topology.pinn.dense import PhysicsInformedNet
...
class FBPINN(torch.nn.Module):
    ...
    def __init__(
        ...
        for loss, polygon in (boundary_loss + [(physic_loss, None)]):
            affected = [i for i, (_, block) in enumerate(self.blocks) if
                        polygon_intersects_rectangle(polygon, block.left_down_corner + block.right_up_corner)]
    ...
    def _build_batched_forward(self) -> None:
        ...
        for layer_idx in range(n_hidden):
            ws = [nn.blocks[layer_idx].w.detach()
                  for nn, _ in self.blocks]
            bss = [nn.blocks[layer_idx].b.detach()
                   for nn, _ in self.blocks]
    ...
    def _manual_forward(
        ...
        for i, (W, b, act) in enumerate(zip(stacked_w, stacked_b, self.layer_activations)):
            # W:   (N, in, out)
            # out: (N, N_pts, in)
    ...
    def custom_compile(
            ...
            loss_func: str | keras.losses.Loss = "MeanSquaredError",
