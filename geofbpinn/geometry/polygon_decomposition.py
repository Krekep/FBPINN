...
class Decomposition2DPolygon(BaseDecomposition):
        ...
        while x < xmax - 1e-12:
            ...
            while y < ymax - 1e-12:
                ...
                if clipped_outer and len(clipped_outer) >= 3:
                    ...
                    kind = (
                        BlockKind.INTERIOR
                        if area_ratio >= 1.0 - self.eps_full and not clipped_holes
                        else BlockKind.BOUNDARY
                    )

                    pts = sample_points_in_polygon_with_holes(
                        clipped_outer, clipped_holes, self.points_per_block
                    )

                    window_fn = self.get_window_function(
                        (rect[0], rect[1]), (rect[2], rect[3])
                    )
                    if pts is None:
                        pts = np.zeros((self.points_per_block, 2), dtype=float)
                    sampler = (
                        polygon_points_sampler(clipped_outer, clipped_holes)
                        if kind == BlockKind.BOUNDARY
                        else None
                    )
                    block_args = (
                        pts.shape,
    ...
    def remove_redundant_blocks(
        ...
        bboxes = np.array(
            [
                (
                    b.left_down_corner[0],
                    b.left_down_corner[1],
                    b.right_up_corner[0],
                    b.right_up_corner[1],
                )
                for b in blocks
        ...
        for idx in order:
            ...
            for j in potential:
                ...
                in_poly = point_in_polygon(px, py, poly_arr)  # (P,)
            ...
            if frac_uncovered <= tol:
                ...
                if verbose:
                    print(
                        f"Removing block {idx}: uncovered fraction {frac_uncovered:.4f} <= tol {tol}"
                    )
            else:
                if verbose:
                    print(
                        f"Keeping block {idx}: uncovered fraction {frac_uncovered:.4f} > tol {tol}"
                    )
