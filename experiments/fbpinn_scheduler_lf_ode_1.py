...
run_name = f"FBPINN_scheduler_{run_id}"
...
layer_scheduler = SequenceLayerScheduler(
    ...
    left_bound_step=3,
    right_bound_step=2,
    left_bound_schedule=20000000,
    right_bound_schedule=20000,
    start_left_bound=0,
    start_right_bound=2,
)
loss_scheduler = LossScheduler(k=500, boundary_indices=list(range(len(ode.sub_losses))))
train_config = {
    "epochs": 160_000,
    "patience": 500_000,
    "eval_interval": 100,
...
mlflow.end_run()
nn.save_weights(f"lf_ode_1_layer_{run_id}.weights.h5")
